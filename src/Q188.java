...
//        System.out.println(prices.length);
        ...
        for (int i = 1; i <= k && i <= prices.length; i++) {
//            dp[0] = Arrays.copyOf(dp[1], prices.length + 1);

            int maxTmp = dp[1] - prices[0];
            int tmp;
            for (int j = 1; j <= prices.length; j++) {
                tmp = Math.max(dp[j - 1], prices[j - 1] + maxTmp);
                maxTmp = Math.max(maxTmp, dp[j] - prices[j - 1]);
                dp[j] = tmp;
            }
        }
